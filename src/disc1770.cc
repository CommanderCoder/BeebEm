...
static void UpdateTR00Status(void)
{
	bool Track0 = (Track == 0);
	...
	if ((Status & WD1770_STATUS_BUSY) && !NMILock) {
		...
		if (FDCommand == 6) { // Read
			...
			if (!(Status & WD1770_STATUS_DATA_REQUEST)) {
				...
				dByteCount--;
				// If reading a single sector, and dByteCount== :-
				// 256..1: read + DRQ (256x)
				//      0: INTRQ + rotate disc
				// If reading multiple sectors, and dByteCount== :-
				// 256..2: read + DRQ (255x)
				//      1: read + DRQ + rotate disc + go back to 256
				if (dByteCount > 0 && !feof(CurrentDisc)) {
					Data = fgetc(CurrentDisc);
					Status |= WD1770_STATUS_DATA_REQUEST;
					NMIStatus |= 1 << nmi_floppy;
				}

				if (dByteCount == 0 || (dByteCount == 1 && MultiSect)) {
					RotSect++;
				...
				if (dByteCount == 1 && MultiSect) {
					dByteCount = SecSize[CurrentDrive] + 1;
					Sector++;
		...
		if (FDCommand == 7 && DWriteable[CurrentDrive]) { // Write
			...
			if (!(Status & WD1770_STATUS_DATA_REQUEST)) {
				...
				dByteCount--;
				// If writing a single sector, and dByteCount== :-
				// 256..2: write + next DRQ (255x)
				//      1: write + INTRQ + rotate disc
				// If writing multiple sectors, and dByteCount== :-
				// 256..2: write + next DRQ (255x)
...
//			ddByteCount--;
//			if (ddByteCount==0) RotSect++; if (RotSect>MaxSects[CurrentDrive]) RotSect=0;
//			if (ddByteCount == 0 && !MultiSect) { ResetStatus(0); NMIStatus |= 1 << nmi_floppy; fseek(CurrentDisc, HeadPos[CurrentDrive], SEEK_SET); FDCommand = 10; } // End of sector
//			if (ddByteCount == 0 && MultiSect) {
//				ddByteCount = 257; Sector++;
...
//	}
	...
	if (FDCommand == 14) {
		...
		if (!(Status & WD1770_STATUS_DATA_REQUEST)) {
			...
			else if (dByteCount == 5) {
				Data = CurrentHead[CurrentDrive];
			}
			else if (dByteCount == 4) {
				Data = RotSect + 1;
			}
			else if (dByteCount == 3) {
			...
			else if (dByteCount == 2) {
				Data = 0;
			}
			else if (dByteCount == 1) {
				Data = 0;
			}
			else if (dByteCount == 0) {
				FDCommand = 0;
				Status &= ~WD1770_STATUS_BUSY;
				RotSect++;
